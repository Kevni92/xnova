...
export const BUILDING_DEFINITIONS = Object.freeze({
  metalMine: definition({
    name: 'Metallmine',
    shortName: 'Metall',
    description: 'Fördert Metall, den wichtigsten Grundstoff für Gebäude und Schiffe.',
    icon: 'M',
  ...
  crystalMine: definition({
    name: 'Kristallmine',
    shortName: 'Kristall',
    description: 'Gewinnt Kristall für Elektronik, Sensoren und komplexe Legierungen.',
    icon: 'K',
    baseCost: { metal: 48, crystal: 24, deuterium: 0 },
    costFactor: 1.6,
    baseDuration: 24,
    effect: 'crystalProduction',
  }),
  deuteriumSynthesizer: definition({
    name: 'Deuteriumsynthetisierer',
    shortName: 'Deuterium',
    description: 'Extrahiert Deuterium als Treibstoff und hochwertigen Energieträger.',
    icon: 'D',
    baseCost: { metal: 225, crystal: 75, deuterium: 0 },
    costFactor: 1.5,
    baseDuration: 32,
    effect: 'deuteriumProduction',
  }),
  solarPlant: definition({
    name: 'Solarkraftwerk',
    shortName: 'Energie',
    description: 'Versorgt Minen und planetare Anlagen mit elektrischer Energie.',
    icon: 'S',
  ...
  metalStorage: definition({
    name: 'Metalllager',
    shortName: 'Metalllager',
    description: 'Erhöht die maximale Lagerkapazität für Metall.',
    icon: 'ML',
  ...
  crystalStorage: definition({
    name: 'Kristalllager',
    shortName: 'Kristalllager',
    description: 'Schützt und lagert größere Mengen Kristall.',
    icon: 'KL',
    baseCost: { metal: 1000, crystal: 500, deuterium: 0 },
    costFactor: 2,
    baseDuration: 40,
    effect: 'crystalStorage',
  }),
  deuteriumTank: definition({
    name: 'Deuteriumtank',
    shortName: 'Deuteriumtank',
    description: 'Erweitert die sichere Lagerkapazität für Deuterium.',
    icon: 'DT',
    baseCost: { metal: 1000, crystal: 1000, deuterium: 0 },
    costFactor: 2,
    baseDuration: 45,
    effect: 'deuteriumStorage',
  }),
  roboticsFactory: definition({
    name: 'Robotikzentrum',
    shortName: 'Robotik',
    description: 'Automatisiert Bauarbeiten und verkürzt zukünftige Bauzeiten.',
    icon: 'R',
...
export function createEmptyBuildings() {
  return Object.fromEntries(BUILDING_KEYS.map((key) => [key, 0]));
}

export function getBuildingDefinition(key) {
  const building = BUILDING_DEFINITIONS[key];
  if (!building) {
    throw new Error(`Unbekanntes Gebäude: ${key}`);
  }
  return building;
}

export function calculateUpgradeCost(key, targetLevel) {
  const building = getBuildingDefinition(key);
  return mapCosts(building.baseCost, (amount) => Math.ceil(amount * building.costFactor ** Math.max(0, targetLevel - 1)));
}

export function calculateBuildDuration(key, targetLevel, roboticsLevel = 0) {
  const building = getBuildingDefinition(key);
  const raw = building.baseDuration * 1.35 ** Math.max(0, targetLevel - 1);
  return Math.max(1, Math.ceil(raw / (1 + Math.max(0, roboticsLevel) * 0.5)));
}

export function calculateDemolitionCost(key, currentLevel) {
  return mapCosts(calculateUpgradeCost(key, currentLevel), (amount) => Math.ceil(amount * DEMOLITION_COST_FACTOR));
}

export function calculateDemolitionDuration(key, currentLevel, roboticsLevel = 0) {
  return Math.max(1, Math.ceil(calculateBuildDuration(key, currentLevel, roboticsLevel) * DEMOLITION_DURATION_FACTOR));
}

export function calculateEconomy(buildings, bonuses = {}) {
  const metalLevel = level(buildings.metalMine);
  const crystalLevel = level(buildings.crystalMine);
  const deuteriumLevel = level(buildings.deuteriumSynthesizer);
  const solarLevel = level(buildings.solarPlant);
  ...
  const energyConsumption = Math.floor(
    8 * metalLevel * 1.1 ** metalLevel
      + 10 * crystalLevel * 1.1 ** crystalLevel
      + 15 * deuteriumLevel * 1.1 ** deuteriumLevel,
  );
  ...
  const production = {
    ...
    deuterium: Math.floor(
      10 * deuteriumLevel * 1.08 ** deuteriumLevel * energyFactor * (1 + bonusFor('deuterium')) * (1 + deuteriumPlanetBonus),
    ),
  };
  ...
  return {
    ...
    energy: {
      ...
      available: energyProduction - energyConsumption,
      factor: energyFactor,
    },
  };
}
...
export function describeBuildingEffect(key, targetLevel, bonuses = {}, baseBuildings = {}) {
  ...
  const economy = calculateEconomy(buildings, bonuses);
  const effect = getBuildingDefinition(key).effect;
...
function definition(value) {
  return Object.freeze({ ...value, baseCost: Object.freeze({ ...value.baseCost }) });
}

function mapCosts(costs, mapper) {
  return Object.fromEntries(COST_KEYS.map((key) => [key, mapper(costs[key] ?? 0, key)]));
}

function storageCapacity(storageLevel) {
  return Math.floor(10_000 * 1.6 ** level(storageLevel));
}
