    ...
    <section class="card auth-grid" data-testid="auth-screen">
      ...
      <div class="forms">
        <form id="register-form" class="panel">
          <h2>Registrieren</h2>
          <label>E-Mail<input data-testid="register-email" name="email" type="email" autocomplete="email" required /></label>
          <label>Passwort<input data-testid="register-password" name="password" type="password" autocomplete="new-password" minlength="8" required /></label>
          <button data-testid="register-submit" type="submit">Account erstellen</button>
        </form>
        <form id="login-form" class="panel">
          <h2>Einloggen</h2>
          <label>E-Mail<input data-testid="login-email" name="email" type="email" autocomplete="email" required /></label>
          <label>Passwort<input data-testid="login-password" name="password" type="password" autocomplete="current-password" minlength="8" required /></label>
  ...
  app.innerHTML = page(`
    <section class="card compact" data-testid="username-screen">
      ...
      <form id="username-form" class="panel">
        <label>Username<input data-testid="username-input" name="username" type="text" minlength="3" maxlength="20" pattern="[A-Za-z0-9_]+" autocomplete="username" required /></label>
...
function renderLoading() { app.innerHTML = page('<section class="card compact centered"><p>Lokaler Spielserver wird gestartet …</p></section>'); }
function renderFatalError(message) { app.innerHTML = page(`<section class="card compact"><h1>Start fehlgeschlagen</h1><p class="notice error">${escapeHtml(message)}</p></section>`); }
function noticeMarkup() { return notice ? `<p class="notice ${notice.type}" data-testid="notice">${escapeHtml(notice.text)}</p>` : ''; }
function page(content, className = 'shell') { return `<div class="${className}">${content}</div>`; }
